...
import com.tedmob.moph.tracer.bluetooth.gatt.ACTION_DEVICE_PROCESSED
import com.tedmob.moph.tracer.bluetooth.gatt.CONNECTION_DATA
import com.tedmob.moph.tracer.bluetooth.gatt.DEVICE_ADDRESS
import com.tedmob.moph.tracer.idmanager.TempIDManager
import com.tedmob.moph.tracer.logging.CentralLog
import com.tedmob.moph.tracer.protocol.BlueTrace
import com.tedmob.moph.tracer.services.BluetoothMonitoringService
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.blacklistDuration
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.maxQueueTime
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.useBlacklist
import java.util.*
...
//                || blacklist.any { it.uniqueIdentifier == work.connectable.manuData }
            ) {
        ...
        if (workQueue.none { it.device.address == work.device.address }) {
            ...
            queueHandler.postDelayed({
                if (workQueue.contains(work))
                    CentralLog.i(
                        TAG,
                        "Work for ${work.device.address} removed from queue? : ${workQueue.remove(work)}"
                    )
            }, maxQueueTime)
            ...
            if (alreadyConnected) {
                ...
                currentWorkOrder.let {
                    ...
                    try {
                        ...
                        it.timeout = System.currentTimeMillis() + BluetoothMonitoringService.connectionTimeout
        ...
        override fun onCharacteristicRead(
            ...
            when (status) {
                BluetoothGatt.GATT_SUCCESS -> {
                    CentralLog.i(
                        TAG,
                        "Characteristic read from ${gatt.device.address}: ${characteristic.getStringValue(0)}"
                    )
                    ...
                    if (BlueTrace.supportsCharUUID(characteristic.uuid)) {
                        try {
                            ...
                            connectionRecord?.let { Utils.broadcastStreetPassReceived(context, connectionRecord) }
                        } catch (e: Throwable) {
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            intent?.let {
                if (ACTION_DEVICE_SCANNED == intent.action) {
                    ...
                    val connectable: ConnectablePeripheral? = intent.getParcelableExtra(CONNECTION_DATA)
