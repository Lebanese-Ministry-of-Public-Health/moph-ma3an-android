...
//                    alertDialog(getString(R.string.verification_failed))
                    ...
                    alertDialog(e.localizedMessage)
                }
            ...
            override fun onCodeAutoRetrievalTimeOut(p0: String) {
                ...
                CentralLog.d(TAG, "Firebase SMS auto retrieval failed.")
                updatePhoneNumberError("Sms auto retrieval failed.")
                onboardingActivityLoadingProgressBarFrame.visibility = View.GONE
            }
        }

    private fun enableFragmentbutton() {
        pagerAdapter?.getItem(pager.currentItem)?.enableButton()
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pager.addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
            ...
            override fun onPageSelected(position: Int) {
                ...
                val onboardingFragment: OnboardingFragmentInterface = pagerAdapter!!.getItem(position)
    ...
    override fun onResume() {
        super.onResume()
        if (mIsOpenSetting) {
            Handler().postDelayed(1_000L) { setupPermissionsAndSettings() }
        }
    }
    ...
    private fun excludeFromBatteryOptimization() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (!powerManager.isIgnoringBatteryOptimizations(packageName)) {
                ...
                if (Utils.canHandleIntent(intent, packageManager)) {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_ENABLE_BT) {
            ...
            if (resultCode != Activity.RESULT_CANCELED) {
                Handler().postDelayed(1_000L) { navigateToNextPage() }
            }
        }
                ...
                for (x in 0 until permissions.size) {
                    ...
                    if (grantResults[x] == PackageManager.PERMISSION_DENIED) {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                            ...
                            } else if (Manifest.permission.WRITE_CONTACTS == permission) {
    ...
    fun navigateToNextPage() {
        ...
        if (!speedUp) {
            ...
            pager.currentItem = 2//force redirect to Consent screen
            pagerAdapter!!.notifyDataSetChanged()
            speedUp = false
        }
    }
        ...
        if (verificationId != null) {
            ...
            try {
                AlertDialog.Builder(this)
                    .setMessage(R.string.auth_validation_id_missing)
    ...
    private inner class ScreenSlidePagerAdapter(fm: FragmentManager) : FragmentStatePagerAdapter(fm) {
        ...
        override fun getItem(position: Int): OnboardingFragmentInterface =
            fragmentMap.getOrPut(position, { createFragAtIndex(position) })

        private fun createFragAtIndex(index: Int): OnboardingFragmentInterface {
            return when (index) {
                0 -> RegisterNumberFragment()
                1 -> OTPFragment()
                2 -> TOUFragment()
                3 -> SetupFragment()
                4 -> SetupCompleteFragment()
                else -> RegisterNumberFragment()
            }
        }
    }
}
