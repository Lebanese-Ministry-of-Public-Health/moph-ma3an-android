...
import com.tedmob.moph.tracer.streetpass.view.RecordViewModel
...
class PeekActivity : AppCompatActivity() {
    ...
    private fun newPeek() {
        ...
        val uidStr = uid?.substring(uid.length - 4) ?: "Firebase not available"
        val serviceUUID = BuildConfig.BLE_SSID
        info.text =
            "UID: $uidStr   SSID: ${serviceUUID.substring(serviceUUID.length - 4)}"
