...
class HomeFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        share_layout.setOnClickListener { shareThisApp() }
        animation_view.setOnClickListener {
            /*if (BuildConfig.DEBUG && ++counter == 2) {
                counter = 0
                context?.startActivity(Intent(context, PeekActivity::class.java))
            }*/
        }
        ...
        remoteConfig.setDefaultsAsync(
            mapOf(
                "ShareText" to getString(R.string.share_message),
                "ShareText_Ar" to getString(R.string.share_message_ar)
            )
        )
    ...
    fun showSetup() {
        ...
        bluetoothText.text = getString(R.string.bluetooth_on_ar) + "\n" + getString(R.string.bluetooth_on)
        locationText.text = getString(R.string.location_on_ar) + "\n" + getString(R.string.location_on)
        batteryText.text = getString(R.string.battery_optimiser_ar) + "\n" + getString(R.string.battery_optimiser)
        pushText.text = getString(R.string.push_noti_ar) + "\n" + getString(R.string.push_noti)
    }
    ...
    override fun onResume() {
        ...
        view?.let {
            ...
            iv_location.isSelected = EasyPermissions.hasPermissions(activity as MainActivity, *perms)

            //push notification
            iv_push.isSelected = NotificationManagerCompat.from(activity as MainActivity).areNotificationsEnabled()
    ...
    private fun shareThisApp() {
        ...
        val shareMessage = when (Preference.getPreferredLanguage(requireActivity())) {
            1 -> remoteConfig.getString("ShareText")
            else -> remoteConfig.getString("ShareText_Ar")
        }
