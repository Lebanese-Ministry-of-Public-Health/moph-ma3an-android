package com.tedmob.moph.tracer.bluetooth.gatt
                ...
                if (BlueTrace.supportsCharUUID(characteristic?.uuid)) {
                    ...
                    characteristic?.uuid?.let { charUUID ->
                        ...
                        if (TempIDManager.bmValid(context)) {
                            ...
                            bluetoothGattServer?.sendResponse(device, requestId, GATT_FAILURE, 0, ByteArray(0))
                        }
                    }
        ...
        override fun onCharacteristicWriteRequest(
            ...
            device?.let {
                ...
                if (BlueTrace.supportsCharUUID(characteristic.uuid)) {
                    ...
                    val valuePassed = value?.let { String(value, Charsets.UTF_8) }.orEmpty()
                    ...
                    if (value != null) {
                        ...
                        if (preparedWrite && responseNeeded) {
                            ...
                            bluetoothGattServer?.sendResponse(device, requestId, GATT_SUCCESS, dataBuffer.size, value)
                        }
        ...
        override fun onExecuteWrite(device: BluetoothDevice, requestId: Int, execute: Boolean) {
            ...
            data.let { dataBuffer ->
                if (dataBuffer != null) {
                    CentralLog.i(
                        TAG,
                        "onExecuteWrite - $requestId- ${device.address} - ${String(dataBuffer, Charsets.UTF_8)}"
                    )
                    saveDataReceived(device)
                    bluetoothGattServer?.sendResponse(device, requestId, GATT_SUCCESS, 0, null)
                } else {
