...
import com.tedmob.moph.tracer.bluetooth.gatt.*
import com.tedmob.moph.tracer.logging.CentralLog
import com.tedmob.moph.tracer.scheduler.Scheduler
import com.tedmob.moph.tracer.services.BluetoothMonitoringService
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.PENDING_ADVERTISE_REQ_CODE
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.PENDING_BM_UPDATE
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.PENDING_HEALTH_CHECK_CODE
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.PENDING_PURGE_CODE
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.PENDING_SCAN_REQ_CODE
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.PENDING_START
import com.tedmob.moph.tracer.status.Status
import com.tedmob.moph.tracer.streetpass.ACTION_DEVICE_SCANNED
import com.tedmob.moph.tracer.streetpass.ConnectablePeripheral
...
object Utils {
    ...
    fun getRequiredPermissions(): Array<String> = arrayOf(Manifest.permission.ACCESS_FINE_LOCATION)
    ...
    fun canHandleIntent(batteryExemptionIntent: Intent, packageManager: PackageManager?): Boolean =
        packageManager?.let { batteryExemptionIntent.resolveActivity(it) != null } ?: false
    ...
    fun scheduleStartMonitoringService(context: Context, timeInMillis: Long) {
        ...
        Scheduler.scheduleServiceIntent(PENDING_START, context, intent, timeInMillis)
    }
    ...
    fun scheduleBMUpdateCheck(context: Context, bmCheckInterval: Long) {
        ...
        Scheduler.scheduleServiceIntent(PENDING_BM_UPDATE, context, intent, bmCheckInterval)
    }
    ...
    fun scheduleNextScan(context: Context, timeInMillis: Long) {
        ...
        Scheduler.scheduleServiceIntent(PENDING_SCAN_REQ_CODE, context, nextIntent, timeInMillis)
    }
    ...
    fun scheduleNextAdvertise(context: Context, timeToNextAdvertise: Long) {
        ...
        //runs every XXX milliseconds
        Scheduler.scheduleServiceIntent(PENDING_ADVERTISE_REQ_CODE, context, nextIntent, timeToNextAdvertise)
    }
    ...
    fun scheduleNextHealthCheck(context: Context, timeInMillis: Long) {
        ...
        Scheduler.scheduleServiceIntent(PENDING_HEALTH_CHECK_CODE, context, nextIntent, timeInMillis)
    }
    ...
    fun scheduleRepeatingPurge(context: Context, intervalMillis: Long) {
        ...
        Scheduler.scheduleRepeatingServiceIntent(PENDING_PURGE_CODE, context, nextIntent, intervalMillis)
    }

    fun broadcastDeviceScanned(context: Context, device: BluetoothDevice, connectableBleDevice: ConnectablePeripheral) {
    ...
    fun getHandShakePin(context: Context, functions: FirebaseFunctions): Task<HttpsCallableResult> {
        return functions
