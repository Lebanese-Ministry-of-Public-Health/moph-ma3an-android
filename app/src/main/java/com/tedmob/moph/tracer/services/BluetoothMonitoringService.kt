package com.tedmob.moph.tracer.services
...
import com.tedmob.moph.tracer.bluetooth.BLEAdvertiser
import com.tedmob.moph.tracer.bluetooth.gatt.ACTION_RECEIVED_STATUS
import com.tedmob.moph.tracer.bluetooth.gatt.ACTION_RECEIVED_STREETPASS
import com.tedmob.moph.tracer.bluetooth.gatt.STATUS
import com.tedmob.moph.tracer.bluetooth.gatt.STREET_PASS
import com.tedmob.moph.tracer.idmanager.TempIDManager
import com.tedmob.moph.tracer.idmanager.TemporaryID
import com.tedmob.moph.tracer.logging.CentralLog
import com.tedmob.moph.tracer.notifications.NotificationTemplates
...
import com.tedmob.moph.tracer.streetpass.StreetPassScanner
import com.tedmob.moph.tracer.streetpass.StreetPassServer
import com.tedmob.moph.tracer.streetpass.StreetPassWorker
...
class BluetoothMonitoringService : Service(), CoroutineScope {
    ...
    private fun notifyLackingThings(override: Boolean = false) {
        if (notificationShown != NOTIFICATION_STATE.LACKING_THINGS || override) {
            val notif = NotificationTemplates.lackingThingsNotification(this.applicationContext, CHANNEL_ID)
    ...
    private fun notifyRunning(override: Boolean = false) {
        if (notificationShown != NOTIFICATION_STATE.RUNNING || override) {
            val notif = NotificationTemplates.getRunningNotification(this.applicationContext, CHANNEL_ID)
            startForeground(NOTIFICATION_ID, notif)
    ...
    private fun isBluetoothEnabled(): Boolean {
        ...
        bluetoothAdapter?.let {
            try {
                ...
                Crashlytics.log("Bluetooth permission not available, most probably due to an app like Knox.")
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (intent == null) {
            CentralLog.e(TAG, "What? Nothing in intent @ onStartCommand")
//            Utils.startBluetoothMonitoringService(applicationContext)
    ...
    private fun setupScanner() {
        streetPassScanner = streetPassScanner ?: StreetPassScanner(this, serviceUUID, scanDuration)
    }
    ...
    private fun scheduleScan() {
        if (!infiniteScanning) {
            commandHandler.scheduleNextScan(
                scanDuration + calcPhaseShift(minScanInterval, maxScanInterval)
            )
        }
    }
