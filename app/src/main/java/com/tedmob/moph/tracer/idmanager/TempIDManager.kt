package com.tedmob.moph.tracer.idmanager
...
import com.tedmob.moph.tracer.services.BluetoothMonitoringService.Companion.bmValidityCheck
import java.io.File
import java.util.*
...
object TempIDManager {
    ...
    fun retrieveTemporaryID(context: Context): TemporaryID? {
        ...
        if (file.exists()) {
            ...
            return getValidOrLastTemporaryID(context, tempIDQueue)
        }
        return null
    }

    private fun getValidOrLastTemporaryID(context: Context, tempIDQueue: Queue<TemporaryID>): TemporaryID {
    ...
    private fun convertToTemporaryIDs(tempIDString: String): Array<TemporaryID> {
        ...
        Crashlytics.log("tempIDString is empty: ${tempIDString.isEmpty() || tempIDString == "[]"}")
    ...
    private fun convertToQueue(tempIDArray: Array<TemporaryID>): Queue<TemporaryID> {
        ...
        val tempIDQueue: Queue<TemporaryID> = LinkedList()
    ...
    fun getTemporaryIDs(context: Context, functions: FirebaseFunctions): Task<HttpsCallableResult> {
        return functions.getHttpsCallable("getTempIDs").call().addOnSuccessListener {
            ...
            if (status.toLowerCase().contentEquals("success")) {
                ...
                Preference.putLastFetchTimeInMillis(context, System.currentTimeMillis())
            }
